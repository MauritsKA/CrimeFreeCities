...
@section('title')
<h1 class="h2">{{ucfirst(__('cmsnav.publications'))}}</h1>
@endsection
...
<form id="upload-form" method="POST" action="{{ url('dashboard/publications')}}" enctype="multipart/form-data">
{{ csrf_field() }}

<div class="row">
    <div class="col-lg-3">
    <div class="form-group">
        <label>Cover publicatie</label><br>
    ...
    <div class="col-lg-3">
    <div class="form-group">
        <label>publicatie</label><br>
        <label class="btn-file">
        <input type="file" name="publication_file" id="publication_file" data-max-size="2097152"></label>
    </div>
    </div>
</div>
...
<table class="table table-hover">
    ...
    <tbody> 
        ...
        <tr>
            <td onclick="document.location.href='{{url('/images')}}/{{$publication->image()->get()->pluck('url')->first()}}';return false;" class="btnextra">

           <div class='image_thumbnails' style="background: url('{{url('/images')}}/{{$publication->image()->get()->pluck('url')->first()}}') no-repeat center center;
...
function clearform(){
    $("#upload-form")[0].reset();
    $("#add").text("Add");
    $('#upload-form').prop('action', '{{ url('dashboard/publications')}}');
    nlsummarymde.value("");
    ensummarymde.value("");
  return false; 
};
...
var url = '{{url('/')}}/dashboard/publications/delete';
